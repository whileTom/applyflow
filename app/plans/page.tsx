...
export default function PlansPage() {
  ...
  const subscriptionProducts = PRODUCTS.filter((p) => p.isSubscription || p.isLifetime)
  const creditProducts = PRODUCTS.filter((p) => p.credits)
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-background via-background to-primary/5">
      <Head>
        <title>Plans & Packages - ApplyFlow</title>
        <meta name="description" content="View all available plans and packages for ApplyFlow." />
      </Head>
      <div className="container mx-auto py-16 px-4 max-w-7xl">
        ...
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-muted-foreground max-w-2xl mx-auto">
            Choose unlimited access or buy credits as you need them.
          </p>
        </div>

        <div className="mb-16">
          <h3 className="text-2xl font-bold text-center mb-8">Subscription Plans</h3>
                ...
                >
                  ...
                  <CardHeader className="text-center pb-2">
                    ...
                    >
                      <Icon
                        className={`w-6 h-6 ${
                          isLifetime ? "text-primary" : isPro ? "text-accent" : "text-muted-foreground"
                        }`}
                      />
                    </div>
                  ...
                  <CardContent className="space-y-4">
                    ...
                    <Button
                      ...
                      variant="default"
                    >
                      {product.isSubscription ? "Subscribe" : product.isLifetime ? "Get Lifetime Access" : "Buy now"}
                    </Button>
                  </CardContent>
                </Card>
              )
        ...
        <div className="mb-16">
          <h3 className="text-2xl font-bold text-center mb-8">Credit Packs</h3>
            ...
            {creditProducts.map((product) => {
              ...
              return (
                <Card
                  key={product.id}
                  className={`rounded-3xl backdrop-blur-xl border-border/50 relative ${
                    isPopular ? "bg-primary/5 border-primary/30 ring-2 ring-primary/20" : "bg-card/50"
                  }`}
                >
                  {product.savings && (
                    <div className="absolute -top-3 left-1/2 -translate-x-1/2">
                  ...
                  <CardContent className="space-y-4">
                    <ul className="space-y-2">
                      {product.features.map((feature) => (
                        <li key={feature} className="flex items-center gap-2">
                          <Check
                            className={`w-4 h-4 flex-shrink-0 ${isPopular ? "text-primary" : "text-muted-foreground"}`}
                          />
                          <span className="text-xs">{feature}</span>
                        </li>
                      ))}
